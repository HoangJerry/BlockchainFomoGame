...
contract Ticket {
    ...
    uint256 constant public LIMMIT_CLOCK_2_ETH = 5 ether;
        ...
        public
    {
        ...
        devTeam4 = _devTeam[3]; 
        currentRound=0;
        round[currentRound].startRound = now;
        round[currentRound].endRoundByClock1 = now.add(3*ONE_HOUR);
    ...
    function drawWinner() public registered() payable returns(address){
        ...
        round[currentRound].startRound = now.add(3*ONE_MIN);
        round[currentRound].totalEth = totalEthLastRound;
        round[currentRound].endRoundByClock1 = now.add(3*ONE_HOUR); //12+48
        round[currentRound].endRound = round[currentRound].endRoundByClock1;
        claim();
    }
    
    function claim() public registered() payable{
    ...
    function buyTicket(uint256 _quantity) payable public registered() buyable() returns(bool) {
        ...
        if (round[currentRound].is_running_clock2==false&&((currentRound==0 && round[currentRound].totalEth>=LIMMIT_CLOCK_2_ETH)||(currentRound>0&&round[currentRound].totalEth>round[currentRound-1].totalEth))){
            ...
            round[currentRound].endRoundByClock2 = now.add(3*ONE_HOUR);
        }
    ...
    function getCitizenSlot(uint256 _round, uint256 _slotId) public view returns(Slot){
